...
class Testimonials_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( $about )
			echo '<p class="about">'.$about. "penis" .'</p>';
...
<style type="text/css">
.testimonials_widget h2 { padding:20px 0 0 98px; margin: 14px 30px 0 0; background: transparent url(<?php echo GPP_TESTIMONIALS_PLUGIN_URL .'/img/quote.png'; ?>) no-repeat top left}
.testimonials_widget h2, .testimonials_widget .about { line-height: 20px; font-style: italic; }
.testimonials_widget .bio { width:50%; margin: 0 auto}
.testimonials_widget img {}
.testimonials_widget .name { display:block; font-size: 14px; font-weight: bold; text-align:right;padding: 20px 100px 0 0;min-width: 90px;min-height: 67px; background: transparent url(<?php echo GPP_TESTIMONIALS_PLUGIN_URL .'/img/quote-end.png'; ?>) no-repeat bottom right}
...
function gpp_testimonials_shortcode($atts, $content = null, $code) {  
    extract(shortcode_atts(array(  
		...
		$testimonial = sprintf( '<div class="row-fluid"><div class="testimonials_widget span12"><h2>%s <span class="name">%s - <p class="about">%s</p></span></h2></div><!-- .testimonials_widget --></div><!-- .row-fluid -->',
			$content, $name, $about);
